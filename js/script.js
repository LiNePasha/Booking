...
// 2- NightsDifference Between Dates

function NightsDifference(d1,d2){
    var time = Math.abs(d2 - d1);
    var days = Math.ceil(time / (1000 * 60 * 60 * 24))
    return days;
}
function call(){
    d1 = new Date(document.getElementById("d1").value);
    d2 = new Date(document.getElementById("d2").value);
    if (d1 == 'Invalid Date' || d2 == 'Invalid Date') {
        return;
    }else {
        document.getElementById("nights").value = NightsDifference(d1,d2);
    }
}